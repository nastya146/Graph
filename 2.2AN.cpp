...
struct EdgeCmp {
  bool operator()(const Edge& frist, const Edge& second) const {
    return frist.from < second.from;
  }
};
...
class Graph {
 public:
  ...
  void Dfs(int16_t vertex, std::queue<Edge>& bridges, int16_t parent = 0) {
    colours_[vertex] = 1;
        ...
        if (colours_[u] == 0) {
          ...
          if (time_in_[vertex] < time_up_[u]) {
            if (std::count(vertex_list_[vertex].begin(), vertex_list_[vertex].end(), u) == 1) {
              bridges.push(Edge{std::min(vertex, u), std::max(vertex, u)});
            }
          }
        }
      }
...
int main() {
  ...
  std::cin >> n >> m;
  Graph graph(n);
  std::map<Edge, int32_t, EdgeCmp> edges;
  ...
  for (int32_t i = 0; i < m; ++i) {
    ...
    edges[Edge{std::min(vert1, vert2), std::max(vert1, vert2)}] = i + 1;
  }
  std::queue<Edge> bridges = graph.Bridges();
